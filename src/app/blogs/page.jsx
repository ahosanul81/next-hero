...
const BlogsPage = () => {
    ...
    return (
        <div className='grid grid-cols-3 gap-4'>
            {
                blogs.map(blog=> <div className='border-2 border-amber-500 p-3 rounded-md'>
                    <h3 className='text-xl font-semibold'>{blog.slug}</h3>
                    <h3 className='text-2xl font-bold'>{blog.title}</h3>
                    <h5 className='text-gray-500'>{blog.description}</h5>
                    <Link href={`/blogs/details/${blog.slug}`}><button className='btn bg-amber-700 text-white'>View Details</button></Link>
                </div>)
            }
        </div>
    );
